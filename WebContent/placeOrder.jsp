...
<body>
	...
	<%
		List<Cart> list3 = CartDAO.getAllRecords(session.getAttribute("email").toString());
		request.setAttribute("list", list3);
		
		Customers list4 = CustomerDAO.getAllRecords(session.getAttribute("email").toString());
		request.setAttribute("list", list4);
		
		OrderDAO.setOrders(list3, list4);
		
		int i = CartDAO.emptyCart(session.getAttribute("email").toString());
		out.println();
		...
		<%
		if(i>0){
			out.println("products removed from cart and item successfully added to orders table");
		}
		else
		{
